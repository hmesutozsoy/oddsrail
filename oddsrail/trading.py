...
async def _client():
    ...
    if _secure is None:
        ...
        _secure = await AsyncSecureClient.create(
            private_key=key,
            wallet=os.environ.get("POLYMARKET_WALLET_ADDRESS", "").strip() or None,
            api_key=api_key,
        )
    return _secure
...
async def place_order(token_id: str, side: str, price: float, size: float,
    ...
    if not math.isfinite(size) or size <= 0:
        raise ValueError("size must be finite and positive (number of shares)")
    ...
    try:
        ...
        async with guard.submission_lock("polymarket"):
            ...
            if open_order_cap is not None:
                try:
                    ...
                    return {**guard.unverified_open_orders(open_order_cap),
                            ...
                            "submitted": intent}
            submit = client.place_limit_order
            block = guard.reserve_live_submission("polymarket", token_id, notional,
...
async def my_balance():
    """Read authenticated collateral for an existing explicitly selected account."""
    from . import local_account

    wallet = operator_wallet()
    # Do not let a read resolve or provision an unspecified default account.
    key = os.environ.get("POLYMARKET_PRIVATE_KEY")
    if hosted.enabled() or not wallet or not key:
        return {**local_account.unavailable_balance(wallet),
                "paper_bankroll_usd": paper.bankroll() if paper.enabled() else None,
                "note": "To read real cash, configure your existing Polymarket account's "
                        "public wallet address and matching signer key in local settings. "
                        "No account was created. In simulation, use paper_positions."}
    return await local_account.read_configured_cash(key, wallet)
