...
async def _post(path: str, body: dict, *, notional_usd: float | None = None,
                open_orders_now: int | None = None):
    if hosted.enabled():
        raise hosted.VenueUnavailable()
    url = base_url() + PREFIX + path
    ...
    async with httpx.AsyncClient(timeout=20.0) as c:
        if notional_usd is not None:
            block = guard.reserve_live_submission("kalshi", body["ticker"],
                                                   notional_usd, open_orders_now)
            if block:
                return block
...
async def _count_open_orders(cap: int) -> int:
    """Verify the complete resting-order count, failing closed on bad pages."""
    total = 0
    cursor = None
    seen_cursors = set()
    async with asyncio.timeout(_OPEN_ORDER_COUNT_TIMEOUT_SECONDS):
        for _ in range(_OPEN_ORDER_COUNT_MAX_PAGES):
            ...
            if cursor:
                params["cursor"] = cursor
            page = await _get("/portfolio/orders", params, signed=True)
            if not isinstance(page, dict) or not isinstance(page.get("orders"), list):
                raise RuntimeError("open-order page has no valid orders list")
            ...
            if not isinstance(cursor, str) or cursor in seen_cursors:
                raise RuntimeError("open-order pagination has an invalid or repeated cursor")
...
async def place_order(ticker: str, outcome: str, action: str, price: float,
    ...
    notional = float(p) * float(count)
    block = guard.check_order("kalshi", ticker, notional, dry_run())
    if block:
    ...
    async with guard.submission_lock("kalshi"):
        open_order_cap = guard._i("ODDSRAIL_MAX_OPEN_ORDERS")
        n_open = None
        if open_order_cap is not None:
            try:
                n_open = await _count_open_orders(open_order_cap)
        ...
        resp = await _post("/portfolio/events/orders", body,
                           notional_usd=notional, open_orders_now=n_open)
        if isinstance(resp, dict) and resp.get("blocked_by") == "guardrail":
            return {**resp, "venue": "kalshi", "ticker": ticker}
