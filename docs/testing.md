...
The hosted service, the website and their tests are developed in a private
repository alongside this package; the public repository carries the MCP
server and its tests only, so a public checkout runs a smaller suite.
...
tests time out after 30 seconds, except the 240-order durable SQLite logical
soak, which allows 120 seconds on shared CI disks. Its lifecycle count,
durability and financial assertions are unchanged; it is not a latency test.
The CI job has a 15-minute limit. JUnit results
are retained for 14 days, including on a failing job.

Supervisor run-loop tests wait for completed steps with a 10-second bound and
always stop their background tasks. They exercise real durable accounting and
cancellation, not a one-second disk-performance target. Separate cadence tests
use a controlled clock to verify quote timing.
